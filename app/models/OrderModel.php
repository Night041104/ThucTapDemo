...
class OrderModel extends BaseModel {
    ...
    // Tạo đơn hàng mới (Có Transaction)
    public function createOrder($userId, $customerData, $cartItems) {
        // Bắt đầu giao dịch bảo vệ dữ liệu
        ...
        try {
            ...
            // Xử lý User ID: Nếu có ID thì bao quanh bởi dấu nháy đơn (vì là UUID string), nếu không thì là NULL
            $uid = $userId ? "'".$this->escape($userId)."'" : 'NULL';
            
            $name  = $this->escape($customerData['fullname']);
            $email = $this->escape($customerData['email']); // [MỚI] Lưu email vào đơn
            $phone = $this->escape($customerData['phone']);
            $addr  = $this->escape($customerData['address']);
            $note  = $this->escape($customerData['note']);

            // C. Tính tổng tiền & Kiểm tra tồn kho lần cuối
            $totalMoney = 0;
            $finalItems = [];
            ...
            // Query lấy giá và tồn kho thực tế từ DB (để tránh hack giá từ session)
            ...
            // [QUAN TRỌNG] Đã thêm cột email
            $sqlOrder = "INSERT INTO orders (order_code, user_id, fullname, email, phone, address, note, total_money, status) 
                         VALUES ('$orderCode', $uid, '$name', '$email', '$phone', '$addr', '$note', '$totalMoney', 1)";
    ...
    public function getOrdersByUserId($userId) {
        $userId = $this->escape($userId); // UUID là chuỗi, chỉ cần escape
    ...
    private function adjustStock($orderId, $operator) {
        ...
        while ($row = mysqli_fetch_assoc($items)) {
            ...
            if ($operator == '+') {
                ...
                $this->conn->query("UPDATE products SET quantity = quantity - $qty WHERE id = '$pid' AND quantity >= $qty");
                if ($this->conn->affected_rows == 0) return false; // Thất bại
            }
        }
        return true;
    }
