<div class="container my-5 shadow-sm p-4 p-md-5 bg-white rounded">
    <h2 class="fw-bold mb-4 text-dark" style="border-left: 5px solid #cd1818; padding-left: 15px;">
        Hướng dẫn mua hàng và thanh toán online
    </h2>

    <div class="content-guide">
        <div class="step-block mb-5">
            <h5 class="fw-bold text-danger">Bước 1: Tìm kiếm sản phẩm bạn muốn</h5>
            <p class="text-justify">
                Đầu tiên, Quý khách vào trang chủ của <strong>FPT Shop Clone System - Uy tín, Chất lượng</strong> tại địa chỉ website của chúng tôi. <br>
                Tại thanh tìm kiếm, gõ tên sản phẩm Quý khách muốn mua (ví dụ: "Iphone 17 Pro Max 256GB"). Hệ thống sẽ hiển thị các sản phẩm liên quan để bạn lựa chọn.
            </p>
            <div class="image-placeholder mt-3 text-center">
                <img src="uploads/shop_guide/buoc1.jpg" class="img-fluid rounded border shadow-sm" alt="Hướng dẫn tìm kiếm">
            </div>
        </div>

        <div class="step-block mb-5">
            <h5 class="fw-bold text-danger">Bước 2: Xem chi tiết sản phẩm</h5>
            <p class="text-justify">
                Sau khi tìm được sản phẩm ưng ý, Quý khách hãy <strong>click vào tên sản phẩm</strong> đó. Một trang mới sẽ hiện ra, hiển thị đầy đủ thông tin chi tiết về sản phẩm, bao gồm thông số kỹ thuật, chính sách bảo hành, và các bài đánh giá để bạn tham khảo.
            </p>
            <div class="image-placeholder mt-3 text-center">
                <img src="uploads/shop_guide/buoc2.jpg" class="img-fluid rounded border shadow-sm" alt="Xem chi tiết">
            </div>
        </div>

        <div class="step-block mb-5">
            <h5 class="fw-bold text-danger">Bước 3: Thêm sản phẩm vào giỏ hàng</h5>
            <p class="text-justify">
                Khi đã quyết định mua, Quý khách chỉ cần click vào nút <span class="badge bg-danger">MUA NGAY</span>. Sản phẩm sẽ được thêm vào giỏ hàng. Quý khách có thể kiểm tra lại các sản phẩm đã chọn bằng cách nhấn vào biểu tượng giỏ hàng trên thanh menu.
            </p>
            <div class="image-placeholder mt-3 text-center">
                <img src="uploads/shop_guide/muangay.jpg" class="img-fluid rounded border shadow-sm" alt="">
            </div>
            <p class="text-justify">Sau khi kiểm tra chính xác sản phẩm muốn đặt mua, quý khách chọn <strong>"Xác nhận đặt hàng"</strong> để tiến hành đặt hàng.</p>
            <div class="image-placeholder mt-3 text-center">
                <img src="uploads/shop_guide/xac_nhan_dat_hang.jpg" class="img-fluid rounded border shadow-sm" alt="Giỏ hàng">
            </div>
        </div>

        <div class="step-block mb-5">
            <h5 class="fw-bold text-danger">Bước 4: Điền thông tin đặt hàng</h5>
            <p class="text-justify">Ở bước này, Quý khách sẽ kiểm tra lại thông tin đơn hàng và cung cấp thông tin cá nhân:</p>
            <ul class="list-unstyled ps-3">
                <li class="mb-2"><strong>1. Hình thức nhận hàng:</strong> Giao hàng tận nơi (điền địa chỉ)</li>
                <li class="mb-2"><strong>2. Thông tin cá nhân:</strong> Nhập chính xác Họ tên, Số điện thoại và ghi chú về đơn hàng để chúng tôi liên hệ giao hàng.</li>
            </ul>
            <div class="image-placeholder mt-3 text-center">
                <img src="uploads/shop_guide/dien_thong_tin.jpg" class="img-fluid rounded border shadow-sm" alt="Điền thông tin">
            </div>
        </div>


        <div class="step-block mb-4">
            <h5 class="fw-bold text-danger">Bước 5: Chọn hình thức thanh toán</h5>
            <p class="text-justify">FPT Shop hỗ trợ 2 phương thức thanh toán linh hoạt. Quý khách có thể lựa chọn tùy theo nhu cầu:</p>
             <div class="image-placeholder mt-3 text-center">
                <img src="uploads/shop_guide/phuong_thuc_thanh_toan.jpg" class="img-fluid rounded border shadow-sm" alt="Các phương thức thanh toán">
            </div>
            
            <div class="bg-light p-3 rounded-3 mb-4 border-start border-danger border-3">
                <div class="mb-3">
                    <h6 class="fw-bold"><i class="fa-solid fa-truck-fast me-2 text-danger"></i>1. Thanh toán khi nhận hàng (COD)</h6>
                    <p class="ms-4 mb-0 text-secondary italic">Sau khi bấm <strong>"Xác nhận đặt hàng"</strong>, hệ thống sẽ hiển thị thông báo đặt hàng thành công hay thất bại ngay lập tức.</p>
                </div>
                <div>
                    <h6 class="fw-bold"><i class="fa-solid fa-credit-card me-2 text-primary"></i>2. Thanh toán Online (Qua VNPAY)</h6>
                    <p class="ms-4 mb-0 text-secondary italic">Hệ thống sẽ chuyển Quý khách đến giao diện thanh toán an toàn của VNPAY để thực hiện quét mã QR hoặc nhập thẻ ATM/Quốc tế.</p>
                </div>
            </div>

            <div class="image-placeholder mt-4 text-center"> 
                <img src="uploads/shop_guide/thanh_toan.jpg" class="img-fluid rounded border shadow-sm" alt="Giao diện thanh toán VNPAY">
            </div>
        </div>

        <div class="alert alert-success mt-5 rounded-3 border-0 shadow-sm p-4">
            <div class="d-flex align-items-center mb-2">
                <i class="fa-solid fa-circle-check fs-4 me-3 text-success"></i>
                <h5 class="fw-bold mb-0 text-success">Hoàn tất đặt hàng</h5>
            </div>
            <hr>
            <p class="mb-0">
                <strong>Đặt hàng thành công!</strong> Đơn hàng của Quý khách đã được hệ thống ghi nhận. Nhân viên của <strong>FPT Shop Clone System</strong> sẽ liên hệ Quý khách qua số điện thoại đã cung cấp trong thời gian sớm nhất để xác nhận đơn và giao hàng.
            </p>
        </div>
</div>

<style>
    .content-guide p {
        line-height: 1.8;
        color: #444;
        margin-top: 10px;
    }
    .image-placeholder img {
        max-width: 100%;
        height: auto;
        margin-bottom: 20px;
    }
    .text-justify {
        text-align: justify;
    }
    .step-block h5 {
        font-size: 1.2rem;
        margin-top: 20px;
    }
</style>